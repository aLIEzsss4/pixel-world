...
// console.log(store, 'store', store.dispatch(setOpen))
...
export default class Game extends Phaser.Scene {
  ...
  async create() {
    ...
    const fish = map.createLayer('fish', FloorAndGround, 0, 0);
    const bar = map.createLayer('bar', FloorAndGround, 0, 0);
    ...
    fish.setCollisionByProperty({
      collides: true
    })
    bg.setCollisionByProperty({
      collides: true
    })
    bar.setCollisionByProperty({
      collides: true
    })
    // debugDraw(groundLayer, this)
    // debugDraw(layer2, this)
    // debugDraw(bg, this)
    ...
    this.physics.add.collider([this.myPlayer, this.myPlayer.playerContainer], bg)
